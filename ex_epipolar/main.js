...
function main() {
	...
	const scene = new Scene(gl, {unlockRoll: true, depthTest: true});
	...
	const P1 = new CoordSystem("P", [Color.RED, Color.GRN, Color.BLU], mat4.multiply(mat4.create(), mat4.fromTranslation(mat4.create(), vec3.fromValues(-3.5,0.07,0)),
																										mat4.fromRotation(mat4.create(), Math.PI/4+0.11, vec3.fromValues(0,1,0))));
	const P2 = new CoordSystem("P'", [Color.RED, Color.GRN, Color.BLU], mat4.multiply(mat4.create(), mat4.fromTranslation(mat4.create(), vec3.fromValues(3.5,0,0)),
																										mat4.fromRotation(mat4.create(), -Math.PI/4-0.05, vec3.fromValues(0,1,0))));
	const X = new Point("X", Color.BLU, vec3.fromValues(-0.23,-3.09,2.95));
	const B = new Segment("Bază", Color.D_PNK, new Point("", Color.BLK, P1.modelMatrix.slice(12,14+1)), new Point("", Color.BLK, P2.modelMatrix.slice(12,14+1)));
	const EP = new Triangle("Planul Epipolar", Color.L_RED, new Point("", Color.BLK, P1.modelMatrix.slice(12,14+1)), new Point("", Color.BLK, P2.modelMatrix.slice(12,14+1)), new Point("", Color.BLK, vec3.fromValues(0.95,-4.25,4.03)));
	...
	const im1Loader = new PlyModelLoader("rect1PlyInput", "im1TexInput", result => {
		...
		const R1 = new Vector("Rază", Color.CYN, new Point("", Color.BLK, vec3.fromValues(6*-0.06, 6*-0.72, 6)));
		...
		const newT3 = vec4.transformMat4(vec4.create(), vec4.fromValues(...result.positions[3], 1),
														P1.modelMatrix).slice(0, 2+1);
		const canonicalT = new Triangle("@@@", Color.GRN, new Point("",Color.BLK,newT1), new Point("",Color.BLK,newT2), new Point("",Color.BLK,newT3));
		//scene.insert(canonicalT.intersect(B, "e", Color.YLW));
		const T = new Triangle("@@@#", Color.GRN, new Point("",Color.BLK,result.positions[1]), new Point("",Color.BLK,result.positions[2]), new Point("",Color.BLK,result.positions[3]))
		P1.add(T.intersect(R1, "x", Color.D_RED));
	...
	const im2Loader = new PlyModelLoader("rect2PlyInput", "im2TexInput", result => {
		...
		const R2 = new Vector("Rază'", Color.YLW, new Point("", Color.BLK, vec3.fromValues(6*-0.06, 6*-0.65, 6)));
		P2.add(R2);
		// Triunghi fictiv pentru intersecții
		...
		const newT3 = vec4.transformMat4(vec4.create(), vec4.fromValues(...result.positions[3], 1),
														P2.modelMatrix).slice(0, 2+1);
		const canonicalT = new Triangle("@@@", Color.GRN, new Point("",Color.BLK,newT1), new Point("",Color.BLK,newT2), new Point("",Color.BLK,newT3));
		//scene.insert(canonicalT.intersect(B, "e'", Color.CYN));
		const I = canonicalT.intersect(EP, "l'", Color.GRN);
		I.p1.position[2] -= 0.01;
		I.p2.position[2] -= 0.01;
		//scene.insert(I)
		const T = new Triangle("@@@#", Color.GRN, new Point("",Color.BLK,result.positions[0]), new Point("",Color.BLK,result.positions[2]), new Point("",Color.BLK,result.positions[3]))
		P2.add(T.intersect(R2, "x'", Color.D_BLU));
